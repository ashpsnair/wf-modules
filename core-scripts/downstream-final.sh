...
OUTDIR=/home/users/nus/ash.ps/scratch/YS-4HNE/downstream/MGO
...
#
# DBS/MNV FIX: Mutect2 emits dinucleotide (DBS) events as a single 2bp
# REF/ALT MNV record, while Strelka's SNV caller only emits them as two
# separate adjacent 1bp SNV records. A plain bcftools isec (exact
# CHROM/POS/REF/ALT match) can never pair these two representations, so
# real DBS events supported by both callers were being silently dropped
# from the consensus (they'd land in the Mutect-only 0000.vcf.gz).
#
# Fix: build ATOMIZED copies of both norm/PASS VCFs (MNVs decomposed
# into their constituent SNVs at each position) purely for the isec
# comparison. Take the shared (0002) positions from that atomized
# isec. Then pull records from the ORIGINAL (non-atomized) Mutect2
# norm/PASS VCF --- preserving native MNV/DBS records --- keeping only
# records for which EVERY base position they span was confirmed shared
# by both callers at the atomic level. This restores DBS calls to the
# consensus without changing SBS/indel behavior at all.
# ------------------------------------------------------------------
step2_consensus() {
  step "STEP 2: normalize + isec consensus (DBS-aware)"
  ...
  while IFS= read -r mutect_vcf; do
    ...
    # ---- atomized copies, used ONLY for isec matching ----
    isec_dir="$OUTDIR/isec/${sample}"
    rm -rf "$isec_dir"; mkdir -p "$isec_dir"
    m_atom="$isec_dir/mutect.atom.vcf.gz"
    s_atom="$isec_dir/strelka.atom.vcf.gz"
    bcftools norm -f "$REF" --atomize --threads "$THREADS" "$m_norm" -Oz -o "$m_atom" 2> "$OUTDIR/logs/${sample}.matom.log"
    bcftools index -f -t --threads "$THREADS" "$m_atom"
    bcftools norm -f "$REF" --atomize --threads "$THREADS" "$s_norm" -Oz -o "$s_atom" 2> "$OUTDIR/logs/${sample}.satom.log"
    bcftools index -f -t --threads "$THREADS" "$s_atom"

    bcftools isec -p "$isec_dir" -c none "$m_atom" "$s_atom" 2> "$OUTDIR/logs/${sample}.isec.log"
    ...
    # atomic positions confirmed shared by both callers
    shared_atom_positions="$isec_dir/shared_atom.positions"
    if [[ -f "$isec_dir/0002.vcf.gz" ]]; then
      bcftools query -f '%CHROM\t%POS\n' "$isec_dir/0002.vcf.gz" | sort -u > "$shared_atom_positions"
    else
      : > "$shared_atom_positions"
    fi

    # For each ORIGINAL (non-atomized) Mutect2 record, keep it only if
    # every position it spans (POS .. POS+len(REF)-1) is in the shared
    # atomic-position set. This preserves native MNV/DBS records while
    # requiring atomic-level agreement from Strelka.
    keep_regions="$isec_dir/consensus_keep.regions"
    bcftools query -f '%CHROM\t%POS\t%REF\n' "$m_norm" \
      | awk -v posfile="$shared_atom_positions" '
          BEGIN{ while ((getline line < posfile) > 0) { shared[line]=1 } }
          { chrom=$1; pos=$2; reflen=length($3); ok=1
            for (k=0; k<reflen; k++) { key=chrom"\t"(pos+k); if (!(key in shared)) { ok=0; break } }
            if (ok) print chrom"\t"pos
          }' > "$keep_regions"

    if [[ -s "$keep_regions" ]]; then
      bcftools view -T "$keep_regions" --threads "$THREADS" "$m_norm" -Oz -o "$OUTDIR/consensus_vcfs/${sample}.consensus.vcf.gz"
    else
    ...
    n_consensus_mnv=$(bcftools view -v mnps -H "$OUTDIR/consensus_vcfs/${sample}.consensus.vcf.gz" 2>/dev/null | wc -l)
    log "  -> mutect_only(atomic)=$n_mutect_only strelka_only(atomic)=$n_strelka_only shared(atomic)=$n_shared | consensus_DBS/MNV_records=$n_consensus_mnv"
    printf "sample\tmutect_only\tstrelka_only\tshared\tconsensus_mnv\n%s\t%d\t%d\t%d\t%d\n" \
      "$sample" "$n_mutect_only" "$n_strelka_only" "$n_shared" "$n_consensus_mnv" > "$OUTDIR/qc/${sample}.counts.tsv"

    rm -f "$m_atom" "$m_atom.tbi" "$s_atom" "$s_atom.tbi" "$shared_atom_positions" "$keep_regions"
  done < "$MUTECT_LIST"
  log "STEP 2 done."
}
