...
# ---- READ GROUP INFO (EDIT AS NEEDED) ----
RGID="WHT463"                  # Read group ID (unique per run/lane)
RGLB="lib1"                    # Library
RGPL="ILLUMINA"                # Platform (typically ILLUMINA)
RGPU="unit1"                   # Platform unit (e.g., flowcell+barcode or lane)
RGSM="WHT463"                  # Sample name (should match tumor sample name)
RG="@RG\tID:${RGID}\tLB:${RGLB}\tPL:${RGPL}\tPU:${RGPU}\tSM:${RGSM}"

########## Alignment with BWA-MEM2 (Tumor only) ########
"$bwa_mem2" mem -t 16 -R "$RG" "$ref_fasta" "$t_f1" "$t_f2" | \
